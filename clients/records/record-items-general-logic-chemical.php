<?php
    ...
    function updatestatus(){
        ...
        $dates = array();
        $expiries = array();
        $status_query = "select date, expirydate from generalstorechemical";
        ...
        for($i = 0; $i < count($dates); $i+=1){          
            ...
            $up_query = "update generalstorechemical set status = timestampdiff(day, '{$queried_now}', '{$expiry}') where date = '{$date}'";
    ...
    $query5 = "select itemname, specification, amount, expirydate from generalstorechemical";
    ...
    if(count($names) === 0){
        ...
        $query = "insert into generalstorechemical (itemname, category, specification, unit,totalamount, amount, expirydate) ";
        ...
        if($item_name === $names[$key1]){
            if($specification === $specs[$key2]){          
                $query7 = "select totalamount, amount from generalstorechemical where itemname = '{$item_name}' and specification = '{$specification}'";
                ...
                $query = "update generalstorechemical set ";
                ...
                $result = mysqli_query($connection, $query);
                if(!$result){
                    echo "Error while updating into generalstorechemical";
                }
                else{
                    echo "<br>";
                ...
                //let's check the number of rows affected
                
            }
            else{
                // $totalprice = $amount * $price;
                $query = "insert into generalstorechemical (itemname, category, specification, totalamount, unit, amount, expirydate) ";
                $query .= "values('{$item_name}', '{$category}', '{$specification}', '{$unit}', '{$amount}', '{$amount}', '{$expirydate}')";
    ...
    for($i = 0; $i < count($dates); $i +=1){          
        ...
        $up_query = "update generalstorechemical set status = timestampdiff(day, '{$date}', '{$expiry}') where date = '{$date}'";
        $up_result = mysqli_query($connection, $up_query);
        if(!$up_result){
            echo "here" . '<br>';
        }

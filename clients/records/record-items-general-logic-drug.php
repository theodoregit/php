<?php
    ...
    function updatestatus(){
        ...
        $dates = array();
        $expiries = array();
        $status_query = "select date, expirydate from generalstoredrugdrug";
        ...
        for($i = 0; $i < count($dates); $i+=1){          
            ...
            $up_query = "update generalstoredrugdrug set status = timestampdiff(day, '{$queried_now}', '{$expiry}') where date = '{$date}'";
    ...
    $query5 = "select itemname, specification, amount, expirydate from generalstoredrug";
    ...
    if(count($names) === 0){
        ...
        $query = "insert into generalstoredrug (itemname, category, specification, unit, totalamount, amount, expirydate) ";
        ...
        if($item_name === $names[$key1]){
            if($specification === $specs[$key2]){            
                $query7 = "select totalamount, amount from generalstoredrug where itemname = '{$item_name}' and specification = '{$specification}'";
                ...
                $query = "update generalstoredrug set ";
                ...
                $result = mysqli_query($connection, $query);
                if(!$result){
                    echo "Error while updating into generalstoredrug";
                }
                else{
                    // echo "<br>";
                ...
                // }
                // else{
                //     $query = "insert into generalstoredrug (itemname, category, specification, unit, amount, price, expirydate) ";
    ...
    $dates = array();
    $expiries = array();
    $status_query = "select date, expirydate from generalstoredrug";
    $status_result = mysqli_query($connection, $status_query);
    while($row = mysqli_fetch_assoc($status_result)){
        $queried_date = $row["date"];
        $queried_expiry = $row["expirydate"];
    ...
    for($i = 0; $i < count($dates); $i +=1){          
        ...
        $up_query = "update generalstoredrug set status = timestampdiff(day, '{$date}', '{$expiry}') where date = '{$date}'";
        $up_result = mysqli_query($connection, $up_query);
        if(!$up_result){
            echo "here" . '<br>';
        }

<?php
    ...
    function updatestatus(){
        ...
        $dates = array();
        $expiries = array();
        $status_query = "select date, expirydate from generalstorefarmtoolfarmtool";
        ...
        for($i = 0; $i < count($dates); $i+=1){          
            ...
            $up_query = "update generalstorefarmtool set status = timestampdiff(day, '{$queried_now}', '{$expiry}') where date = '{$date}'";
    ...
    $query5 = "select itemname, specification, amount, expirydate from generalstorefarmtool";
    ...
    if(count($names) === 0){
        ...
        $query = "insert into generalstorefarmtool (itemname, category, specification, unit, totalamount, amount, expirydate) ";
        $query .= "values('{$item_name}', '{$category}', '{$specification}', '{$unit}', '{$amount}', '{$amount}', '{$expirydate}')";
        ...
        if($item_name === $names[$key1]){
            if($specification === $specs[$key2]){            
                $query7 = "select totalamount, amount from generalstorefarmtool where itemname = '{$item_name}' and specification = '{$specification}'";
                $result7 = mysqli_query($connection, $query7);
                while($row = mysqli_fetch_assoc($result7)){
                    $queried_amountt = $row["amount"];
                    $queried_amounttt = $row["totalamount"];
                    // $queried_price = $row["price"];
                }
                $new_amount = $amount + $queried_amountt;
                $new_amountt = $amount + $queried_amounttt;
                // $new_price = (($queried_amount * $queried_price) + ($amount * $price)) / $new_amount;
                // $totalprice = $new_amount * $new_price;
                $query = "update generalstorefarmtool set ";
                $query .= "totalamount = '{$new_amountt}', amount = '{$new_amount}', expirydate = '{$expirydate}' where itemname = '{$item_name}' and specification = '{$specification}'";
                $result = mysqli_query($connection, $query);
                if(!$result){
                    echo "Error while updating into generalstorefarmtool";
                }
                else{
                    // echo "<br>";
                ...
                // }
                // else{
                //     $query = "insert into generalstorefarmtool (itemname, category, specification, unit, amount, price, expirydate) ";
    ...
    $dates = array();
    $expiries = array();
    $status_query = "select date, expirydate from generalstorefarmtool";
    $status_result = mysqli_query($connection, $status_query);
    while($row = mysqli_fetch_assoc($status_result)){
        $queried_date = $row["date"];
        $queried_expiry = $row["expirydate"];
    ...
    for($i = 0; $i < count($dates); $i +=1){          
        ...
        $up_query = "update generalstorefarmtool set status = timestampdiff(day, '{$date}', '{$expiry}') where date = '{$date}'";
        $up_result = mysqli_query($connection, $up_query);
        if(!$up_result){
            echo "here" . '<br>';
        }

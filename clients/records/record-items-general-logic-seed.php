<?php
    ...
    function updatestatus(){
        ...
        $dates = array();
        $expiries = array();
        $status_query = "select date, expirydate from generalstoreseedseed";
        ...
        for($i = 0; $i < count($dates); $i+=1){          
            ...
            $up_query = "update generalstoreseed set status = timestampdiff(day, '{$queried_now}', '{$expiry}') where date = '{$date}'";
    ...
    $query5 = "select itemname, specification, amount, expirydate from generalstoreseed";
    ...
    if(count($names) === 0){
        ...
        $query = "insert into generalstoreseed (itemname, category, specification, unit, totalamount, amount, expirydate) ";
        ...
        if($item_name === $names[$key1]){
            if($specification === $specs[$key2]){            
                $query7 = "select totalamount, amount from generalstoreseed where itemname = '{$item_name}' and specification = '{$specification}'";
                ...
                $query = "update generalstoreseed set ";
                ...
                $result = mysqli_query($connection, $query);
                if(!$result){
                    echo "Error while updating into generalstoreseed";
                }
                else{
                    // echo "<br>";
                ...
                // }
                // else{
                //     $query = "insert into generalstoreseed (itemname, category, specification, unit, amount, price, expirydate) ";
    ...
    $dates = array();
    $expiries = array();
    $status_query = "select date, expirydate from generalstoreseed";
    $status_result = mysqli_query($connection, $status_query);
    while($row = mysqli_fetch_assoc($status_result)){
        $queried_date = $row["date"];
        $queried_expiry = $row["expirydate"];
    ...
    for($i = 0; $i < count($dates); $i +=1){          
        ...
        $up_query = "update generalstoreseed set status = timestampdiff(day, '{$date}', '{$expiry}') where date = '{$date}'";
        $up_result = mysqli_query($connection, $up_query);
        if(!$up_result){
            echo "here" . '<br>';
        }

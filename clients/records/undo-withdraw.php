<?php
    // include 'record-saled-logic.php';
    $connection = mysqli_connect("localhost", "root", "", "farm");

    session_start();

    //check the connectivity
    if(mysqli_connect_errno()){
        die("Database connection failed: " . mysqli_connect_error() . "(" . mysqli_connect_errno() . ")");
    }

    $main_cat = $_POST["main_cat"];
    switch($main_cat){
        case "Chemical":
            $main = "chemical";
            break;
        case "Seed":
            $main = "seed";
            break;
        case "Farm Tool":
            $main = "farmtool";
            break;
        case "Fertilizer":
            $main = "fertilizer";
            break;
        case "Drug":
            $table = "drug";
            break;
    }


    // $new_amount =  $_SESSION['export_amount'];
    $table_withdraw = "withdraw" . $table;
    $table_general = "generalstore" . $table;
    $query5 = "delete from $table_withdraw order by id desc limit 1";
    $result5 = mysqli_query($connection, $query5);
    if(!$result5){
        echo "error";
    }
    else{
        //let's update other tables affected by the undo action
        $new_amount =  $_SESSION['e_amount'];
        $new_itemname =  $_SESSION['e_itemname'];
        $new_spec =  $_SESSION['e_spec'];
        $query3 = "select amount from $table_general where itemname = '{$new_itemname}' and specification = '{$new_spec}'";
        $result3 = mysqli_query($connection, $query3);        
        while($row = mysqli_fetch_assoc($result3)){            
            $queried_amt = $row["amount"];
        }
        echo $new_amount;
        $new_amounttt = $new_amount + $queried_amt;
        $query = "update $table_general set amount = '{$new_amounttt}' where itemname = '{$new_itemname}' and specification = '{$new_spec}'";
        $result = mysqli_query($connection, $query);
        if(!$result){
            echo "Error";
        }
        else{
            header("Location: record-withdraw-" . $table . "s.php");
        }

    }

    $drop1 = "alter table withdraw drop id";
    $result_drop1 = mysqli_query($connection, $drop1);
    $auto1 = "alter table withdraw auto_increment = 1";
    $result_auto1 = mysqli_query($connection, $auto1);
    $add1 = "alter table withdraw add id int not null auto_increment primary key first";
    $result_add1 = mysqli_query($connection, $add1);

      

    mysqli_close($connection);
?>
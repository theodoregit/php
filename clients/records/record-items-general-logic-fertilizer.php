<?php
    ...
    function updatestatus(){
        ...
        $dates = array();
        $expiries = array();
        $status_query = "select date, expirydate from generalstorefertilizer";
        ...
        for($i = 0; $i < count($dates); $i+=1){          
            ...
            $up_query = "update generalstorefertilizer set status = timestampdiff(day, '{$queried_now}', '{$expiry}') where date = '{$date}'";
    ...
    $query5 = "select itemname, specification, amount, expirydate from generalstorefertilizer";
    ...
    if(count($names) === 0){
        ...
        $query = "insert into generalstorefertilizer (itemname, category, specification, unit, totalamount, amount, expirydate) ";
        ...
        if($item_name === $names[$key1]){
            if($specification === $specs[$key2]){            
                $query7 = "select totalamount, amount from generalstorefertilizer where itemname = '{$item_name}' and specification = '{$specification}'";
                ...
                $query = "update generalstorefertilizer set ";
                ...
                $result = mysqli_query($connection, $query);
                if(!$result){
                    echo "Error while updating into generalstorefertilizer";
                }
                else{
                    // echo "<br>";
                ...
                // }
                // else{
                //     $query = "insert into generalstorefertilizer (itemname, category, specification, unit, amount, price, expirydate) ";
    ...
    for($i = 0; $i < count($dates); $i +=1){          
        ...
        $up_query = "update generalstorefertilizer set status = timestampdiff(day, '{$date}', '{$expiry}') where date = '{$date}'";
        $up_result = mysqli_query($connection, $up_query);
        if(!$up_result){
            echo "here" . '<br>';
        }
